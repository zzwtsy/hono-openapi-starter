...
| [ADR-0004: 权限层自建，不扩展 Better Auth](0004-authorization-layer.md) | Accepted | 权限层（组织树+RBAC+直接授权+过期+deny）自建为独立模块，不做 Better Auth 插件，Better Auth 只管认证。 | [权限层规范](../conventions/authorization.md)、[Better Auth 集成](../conventions/auth-better-auth.md) |
