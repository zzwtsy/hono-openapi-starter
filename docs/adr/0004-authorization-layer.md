...
- **core 零 import features**：权限 catalog 用 declaration merging（各 feature `permissions.ts` 用 `definePermissionCatalog()` 生成完整数组，并以 `declare module` 注册到 core 的 `AppPermissionRegistry`；`AppPermissionCode` 从 registry 推导；当前组装点 `catalogs/permissions.ts` 汇总并执行 `AllPermissionsCovered`/`NoUnknownPermissions` 校验）。权限 code 与 `PermissionRef` 展示元数据分离，DB `permissions` 只保留 code-only 外键 registry。路由由 `app/register-features.ts` 挂载（`core/app` 只挂认证路由）。core 不再 import features，[directory-structure.md](../architecture/backend/directory-structure.md) 的“core 不能 import features”完全成立。契约细节见 [ADR-0011](0011-permission-contract-separation.md)。
