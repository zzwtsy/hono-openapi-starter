...
- 全局审计页:桌面 TanStack Table v9 + 移动事件摘要列表、服务端 offset 分页、筛选(actions 多选/结果/操作人名称/时间范围)、手动刷新、显式详情按钮与结构化 diff
...
features/audit/
  components/
    audit-log-table.tsx          # 全局审计页:search 驱动 + 响应式视图 + 分页/列配置
    audit-log-data-table.tsx     # 桌面语义表格，详情使用单元格内真实 Button
    audit-log-mobile-list.tsx    # 移动事件摘要列表
    audit-log-filters.tsx        # 桌面筛选条
    audit-filter-sheet.tsx       # 移动筛选草稿，应用时一次写 URL
    audit-filter-chips.tsx       # 已生效筛选条件与单项清除
    audit-compact-pagination.tsx # 移动上一页/下一页紧凑分页
    audit-empty-state.tsx        # AuditEmptyState(有筛选无结果 / 首次无数据二分)
    audit-log-detail-sheet.tsx   # 事件摘要/对象/业务 diff 优先，技术详情默认折叠
...
- **移动筛选**:`<768px` 用 Sheet 承载本地筛选草稿；关闭/取消不改变 URL，点击“应用筛选”才一次提交。工具栏下方用 Chip 展示已生效条件，可逐项清除；分页收敛为上一页/下一页和当前页摘要。
...
页面布局遵循 [前端 Data Table 约定](../../conventions/frontend/data-table.md)：桌面表体滚动、表头吸顶、分页栏位于 viewport 外常驻；移动端切换为事件摘要列表，避免横向表格。桌面行保持原生 `<tr>` 语义，不承担点击或键盘入口，固定尾列中的真实“查看详情”按钮负责打开 Sheet。列显隐仅改变 UX，不承担敏感字段保护。

## 详情信息层级

详情 Sheet 先回答“谁在何时对什么做了什么、结果如何”：顶部展示中文 action label、状态和自然语言事件摘要，其后是关联对象与业务字段 diff。常见字段名和 `disabled`/`effect` 等枚举转为业务文案，基础数组按新增/移除展示。action code、IP、request ID、User-Agent、记录时间和原始 JSON 收进默认折叠的“技术详情”。用户、角色、组织资源在具备对应 `*.read` 权限时提供 IAM 导航；无权限或未知资源保持纯文本。
