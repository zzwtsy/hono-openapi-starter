...
权限在 `features/projects/permissions.ts` 用 `definePermissionCatalog()` 声明，由 `catalogs/permissions.ts` 汇总为 `allPermissions`（`AppPermissionCode` 从 feature registry 推导；展示元数据由 `PermissionRef` 提供）。
