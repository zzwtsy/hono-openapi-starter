...
apps/frontend/src/
  ...
  components/
    ...
    shared/               # 自定义跨 feature 复用组件(PageHeader/ListSkeleton/DatePicker)
...
自定义跨 feature 复用的展示组件(PageHeader、ListSkeleton、DatePicker)。非 shadcn 生成,可自由修改;依赖范围同 `ui`(ui/lib/types/hooks)。
...
| from | 允许 to |
| --- | --- |
| routes | features, lib, ui, api, layout, shared, hooks, types |
| features | lib, ui, api, shared, hooks, types(features 之间 disallow,跨 feature 走 routes 装配) |
| lib | lib, types |
...
| layout | features, lib, ui, shared, hooks, types(装配层:侧边栏/页头 shell,接 logout、消费 AuthState) |
| shared | shared, ui, lib, types, hooks(自定义复用组件,依赖范围同 ui) |
...
4. `layout` 是页面级 shell 装配(AppSidebar/auth-layout);页面内容区的自定义展示组件(如 PageHeader)归 `shared`,shadcn 生成组件归 `ui`,供 features 合法引用。
