---
name: feature-pr
description: 在 feature/fix 分支上把已验证全绿的改动提成 GitHub PR 合并到 main。当用户说"创建 PR/提 PR/发 pull request/合并到 main/准备合入"时触发。重点用 --body-file 规避中文 body 的 shell 编码问题。
---

# Feature 分支 PR

feature/fix 都走"分支 -> 验证全绿 -> PR 合并 main"。本 skill 固化流程和 PR body 格式，重点解决 `gh pr create` 中文 body 的编码重试问题。

## 前置条件

- 改动已在分支上完成，**验证门禁全绿**。门禁没全绿禁止提 PR。
...
一个大改造拆成多个顺序 PR 时：每个 PR 一个分支，前一个合并后 `git switch main && git pull --ff-only` 再切下一个。

## 提交

- 代码和文档分别 commit（文档收敛单独一个 commit）。用显式文件列表 `git add <files>`，禁止 `git add -A`。
- 标题用 Conventional commit：`<type>(<scope>): <摘要>`，type 用 feat/fix/refactor/docs/chore/test/ci/build。
- body 用中文时用多个 `-m`：`git commit -m "标题" -m "正文要点"`。
- squash 合并时 GitHub 会自动在标题追加 `(#N)`，禁止手写编号。

## PR body（用 --body-file，禁止内联）
...
为什么做这个改动，一两句。承接哪个计划/PR（链接 `docs/plans/xxx.md` 或上个 PR）。
