---
name: execution-plan
description: 为跨阶段、跨模块或高风险改动起草执行计划。当用户说"写计划/写执行计划/规划/拆解实现/写落地方案/写下修复计划"或要开始一个多步骤、跨 feature 的改动时触发。小改动（单文件、改个 bug）不触发。
---

# 执行计划起草

跨阶段、跨模块或高风险改动应先维护执行计划。本 skill 提供计划模板和验证门禁链。

## 何时用 / 何时不用

- **用**：跨 feature、跨 core/db、引入新依赖、改 schema/migration、改架构边界、多步骤重构、审查后落修复方案。
- **不用**：单文件小改、改注释、改文案、一个明确 bug 的直接修复--直接动手，禁止为小改动写计划。

## 计划文件放哪

路径：`docs/plans/<kebab-name>.md`。
...
- 引用 ADR / convention / 当前源码作为依据，标题保留"基于 X，非推测"签名，提醒避免脑补。
- 多决策时用表格（决策 / 结论 / 理由）。
- schema / API 变更用表格列字段。
...
2. **查事实**：用 `rg` / `find` 核对当前源码、schema、测试、配置--"基于 X，非推测"，禁止拿旧文档当事实。
3. **起草**：用上面模板写 `docs/plans/<name>.md`。`related` 填真正相关的文档路径。
4. **镜像到 TodoWrite**：把步骤/阶段映成 todo，**结尾永远保留两条**：一条"文档同步"、一条"验证（门禁全绿）"。验证 todo 只有在所有门禁通过后才标完成。
